...
inline double delta_roma(double r) {
  ...
  if (ar <= 0.5) {
    // Guard: 1 - 3r² can be -ε when |r| is exactly 0.5 due to FP rounding
...
inline double delta_peskin4(double r) {
  ...
  if (ar <= 2.0) {
    // Guard: -7 + 12ar - 4ar² can be -ε at ar = 2.0 boundary
...
public:
  ...
  static inline void phys_to_local(const Grid<Lattice>& g, double xp, double yp, double zp,
                                   const double lo[3], double& lx, double& ly, double& lz) {
    lx = (xp-lo[0])/g.dx - g.offset[0] + 1.0;
    ly = (yp-lo[1])/g.dx - g.offset[1] + 1.0;
    lz = (Lattice::D==3) ? ((zp-lo[2])/g.dx - g.offset[2] + 1.0) : 0.0;
  }
  ...
  {
    double lx, ly, lz;
    phys_to_local(g, xp, yp, zp, lo, lx, ly, lz);
      ...
      DeltaKernel kern = DeltaKernel::ROMA)
  {
    ...
    const int i0=(int)std::floor(lx), j0=(int)std::floor(ly);
    const int k0=(Lattice::D==3)?(int)std::floor(lz):0;
    const double dvr = dvl / (g.dx*g.dx*((Lattice::D==3)?g.dx:1.0));
